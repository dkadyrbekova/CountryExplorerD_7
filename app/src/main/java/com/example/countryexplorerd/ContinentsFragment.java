...
public class ContinentsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        // Настройка карусели (RecyclerView)
        rvSightseeing = view.findViewById(R.id.rvSightseeing);
        if (rvSightseeing != null) {
            rvSightseeing.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.HORIZONTAL, false));
        }

        // Подключение ViewModels
    ...
    private void setupSightseeingCarousel() {
        sightseeingViewModel.getSightseeingList().observe(getViewLifecycleOwner(), sights -> {
            if (sights != null && !sights.isEmpty()) {
                SightseeingAdapter adapter = new SightseeingAdapter(sights, item -> {
                    // Открываем детали при клике на карточку
                    showSightseeingDetails(item);
                });
    ...
    // МЕТОД ПОКАЗА ДЕТАЛЕЙ (С КНОПКОЙ ЗАКРЫТИЯ)
    private void showSightseeingDetails(Sightseeing item) {
        ...
        Button btnClose = view.findViewById(R.id.btnCloseDialog); // Кнопка "Назад"
    ...
    private Country getOrGenerateCountryOfDay(List<Country> countries) {
        ...
        if (savedDay == today) {
            ...
            for (Country c : countries) {
                if (c.getName().equals(savedCountryName)) return c;
            }
        }

        int randomIndex = today % countries.size();
